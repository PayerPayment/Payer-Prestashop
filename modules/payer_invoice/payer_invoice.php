...
class Payer_invoice extends PaymentModule {
	public function __construct(){
		$this->name = 'payer_invoice';
		...
		$this->displayName = $this->l('Payer Invoice');
		$this->description = $this->l('Pay by invoice with payer.');
	}
	public function install(){
		if (!parent::install() 
				OR !Configuration::updateValue('PAYER_INVOICE_TEST', 1)
				OR !Configuration::updateValue('PAYER_INVOICE_AGENTID', 1) 
				OR !Configuration::updateValue('PAYER_INVOICE_KEY1', 1)
				OR !Configuration::updateValue('PAYER_INVOICE_KEY2', 1) 
	...
	public function uninstall(){
		if(!parent::uninstall() OR !Configuration::deleteByName('PAYER_INVOICE_TEST') OR !Configuration::deleteByName('PAYER_INVOICE_AGENTID') OR !Configuration::deleteByName('PAYER_INVOICE_KEY1') OR !Configuration::deleteByName('PAYER_INVOICE_KEY2'))
			return false;
		return true;
	}
	...
	public function getContent(){
		...
		if (isset($_POST['submitSecurepay'])){
			...
			if (!sizeof($this->_postErrors)){
				Configuration::updateValue('PAYER_INVOICE_AGENTID', $_POST['agentid']);
				Configuration::updateValue('PAYER_INVOICE_KEY1', $_POST['key1']);
				Configuration::updateValue('PAYER_INVOICE_KEY2', $_POST['key2']);
				Configuration::updateValue('PAYER_INVOICE_TEST', intval($_POST['test']));
				$this->displayConf();
			}
			else {
				$this->displayErrors();
	...
	public function displayFormSettings(){
		$conf = Configuration::getMultiple(array('PAYER_INVOICE_TEST', 'PAYER_INVOICE_AGENTID','PAYER_INVOICE_KEY1','PAYER_INVOICE_KEY2'));
		$payertest = array_key_exists('test', $_POST) ? $_POST['test'] : (array_key_exists('PAYER_INVOICE_TEST', $conf) ? $conf['PAYER_INVOICE_TEST'] : '');
		$agentid = array_key_exists('agentid', $_POST) ? $_POST['agentid'] : (array_key_exists('PAYER_INVOICE_AGENTID', $conf) ? $conf['PAYER_INVOICE_AGENTID'] : '');
		$key1 = array_key_exists('key1', $_POST) ? $_POST['key1'] : (array_key_exists('PAYER_INVOICE_KEY1', $conf) ? $conf['PAYER_INVOICE_KEY1'] : '');
		$key2 = array_key_exists('key2', $_POST) ? $_POST['key2'] : (array_key_exists('PAYER_INVOICE_KEY2', $conf) ? $conf['PAYER_INVOICE_KEY2'] : '');
	...
	public function hookPayment($params){
		...
		$payerApi->add_payment_method("invoice");
		if(Configuration::get('PAYER_INVOICE_TEST')=='1'){
