...
class Payer_all extends PaymentModule {
    public function __construct(){
        ...
        $this->displayName = $this->l('All payment methods');
        $this->description = $this->l('Pay by directbank, card, invoice or part payment with Payer.');
    }
    public function install(){
        if (!parent::install() 
        		OR !Configuration::updateValue('PAYER_ALL_TEST', 1) 
        		OR !Configuration::updateValue('PAYER_ALL_AGENTID', 1) 
        		OR !Configuration::updateValue('PAYER_ALL_KEY1', 1) 
        		OR !Configuration::updateValue('PAYER_ALL_KEY2', 1) 
    ...
    public function uninstall(){
        if(!parent::uninstall() 
        		OR !Configuration::deleteByName('PAYER_ALL_TEST') 
        		OR !Configuration::deleteByName('PAYER_ALL_AGENTID') 
        		OR !Configuration::deleteByName('PAYER_ALL_KEY1') 
        		OR !Configuration::deleteByName('PAYER_ALL_KEY2'))
        return false;
        return true;
    }
    ...
    public function getContent(){
        ...
        if (isset($_POST['submitSecurepay'])){
            ...
            if (!sizeof($this->_postErrors)){
                Configuration::updateValue('PAYER_ALL_AGENTID', $_POST['agentid']);
                Configuration::updateValue('PAYER_ALL_KEY1', $_POST['key1']);
                Configuration::updateValue('PAYER_ALL_KEY2', $_POST['key2']);
                Configuration::updateValue('PAYER_ALL_TEST', intval($_POST['test']));
                $this->displayConf();
            }
            else {
                $this->displayErrors();
    ...
    public function displayFormSettings(){
        $conf = Configuration::getMultiple(array('PAYER_ALL_TEST', 'PAYER_ALL_AGENTID','PAYER_ALL_KEY1','PAYER_ALL_KEY2'));
        $payertest = array_key_exists('test', $_POST) ? $_POST['test'] : (array_key_exists('PAYER_ALL_TEST', $conf) ? $conf['PAYER_ALL_TEST'] : '');
        $agentid = array_key_exists('agentid', $_POST) ? $_POST['agentid'] : (array_key_exists('PAYER_ALL_AGENTID', $conf) ? $conf['PAYER_ALL_AGENTID'] : '');
        $key1 = array_key_exists('key1', $_POST) ? $_POST['key1'] : (array_key_exists('PAYER_ALL_KEY1', $conf) ? $conf['PAYER_ALL_KEY1'] : '');
        $key2 = array_key_exists('key2', $_POST) ? $_POST['key2'] : (array_key_exists('PAYER_ALL_KEY2', $conf) ? $conf['PAYER_ALL_KEY2'] : '');
    ...
    public function hookPayment($params){
        ...
        $payerApi->add_payment_method("auto");
        if(Configuration::get('PAYER_ALL_TEST')=='1'){
