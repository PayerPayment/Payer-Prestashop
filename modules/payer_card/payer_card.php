...
class Payer_card extends PaymentModule {
	public function __construct(){
		$this->name = 'payer_card';
		...
		$this->displayName = $this->l('Payer Card');
		$this->description = $this->l('pay by card with payer.');
	}
	public function install(){
		if (!parent::install() 
				OR !Configuration::updateValue('PAYER_CARD_TEST', 1)
				OR !Configuration::updateValue('PAYER_CARD_AGENTID', 1) 
				OR !Configuration::updateValue('PAYER_CARD_KEY1', 1)
				OR !Configuration::updateValue('PAYER_CARD_KEY2', 1) 
	...
	public function uninstall(){
		if(!parent::uninstall() OR !Configuration::deleteByName('PAYER_CARD_TEST') OR !Configuration::deleteByName('PAYER_CARD_AGENTID') OR !Configuration::deleteByName('PAYER_CARD_KEY1') OR !Configuration::deleteByName('PAYER_CARD_KEY2'))
			return false;
		return true;
	}
	...
	public function getContent(){
		...
		if (isset($_POST['submitSecurepay'])){
			...
			if (!sizeof($this->_postErrors)){
				Configuration::updateValue('PAYER_CARD_AGENTID', $_POST['agentid']);
				Configuration::updateValue('PAYER_CARD_KEY1', $_POST['key1']);
				Configuration::updateValue('PAYER_CARD_KEY2', $_POST['key2']);
				Configuration::updateValue('PAYER_CARD_TEST', intval($_POST['test']));
				$this->displayConf();
			}
			else {
				$this->displayErrors();
	...
	public function displayFormSettings(){
		$conf = Configuration::getMultiple(array('PAYER_CARD_TEST', 'PAYER_CARD_AGENTID','PAYER_CARD_KEY1','PAYER_CARD_KEY2'));
		$payertest = array_key_exists('test', $_POST) ? $_POST['test'] : (array_key_exists('PAYER_CARD_TEST', $conf) ? $conf['PAYER_CARD_TEST'] : '');
		$agentid = array_key_exists('agentid', $_POST) ? $_POST['agentid'] : (array_key_exists('PAYER_CARD_AGENTID', $conf) ? $conf['PAYER_CARD_AGENTID'] : '');
		$key1 = array_key_exists('key1', $_POST) ? $_POST['key1'] : (array_key_exists('PAYER_CARD_KEY1', $conf) ? $conf['PAYER_CARD_KEY1'] : '');
		$key2 = array_key_exists('key2', $_POST) ? $_POST['key2'] : (array_key_exists('PAYER_CARD_KEY2', $conf) ? $conf['PAYER_CARD_KEY2'] : '');
	...
	public function hookPayment($params){
		...
		$payerApi->add_payment_method("card");
		if(Configuration::get('PAYER_CARD_TEST')=='1'){
		...
		$Auth_url = stripslashes("http://" . _PS_BASE_URL_.__PS_BASE_URI__ . "/modules/payer__common/auth.php?id_cart=".intval($params['cart']->id).'&id_order_status=2&amount='.$amount;
		$Settle_url = stripslashes("http://" . _PS_BASE_URL_.__PS_BASE_URI__ . "/modules/payer__common/settle.php?id_cart=".intval($params['cart']->id).'&id_order_status=2&amount='.$amount;
		$Shop_url = stripslashes("http://" . _PS_BASE_URL_.__PS_BASE_URI__ .'/index.php?controller=history'.'&id_cart='.intval($params['cart']->id).'&id_module='.$this->id;
